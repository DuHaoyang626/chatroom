...
public interface ChatFriendApplyDao extends BaseMapper<ChatFriendApply> {
    ...
    @Select("select cfa.id, cfa.apply_user_id,cfa.friend_id, cu.phone as nickname,cfaur.remark,cu.avatar,cu.signature,cfaur.unread_count, cfa.status, cfa.apply_reply,cfa.apply_reason " +
    ...
    @Select("select cfa.id, cfa.apply_user_id,cfa.friend_id, cu.phone as nickname,cfaur.remark,cu.avatar,cu.signature,cfaur.remark, cfa.status, cfaur.label,cfa.source ,cfaur.unread_count " +
            "from chat_friend_apply cfa " +
            "         inner join chat_friend_apply_user_rel cfaur on cfa.id = cfaur.apply_id " +
            "         inner join chat_user cu on cu.id = cfaur.friend_id " +
