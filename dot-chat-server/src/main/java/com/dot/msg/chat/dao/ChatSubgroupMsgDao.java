...
public interface ChatSubgroupMsgDao extends BaseMapper<ChatSubgroupMsg> {
     ...
     * 获取小组消息列表（最新的消息）
     */
    @Select("SELECT * FROM chat_subgroup_msg WHERE subgroup_id = #{subgroupId} ORDER BY send_time DESC LIMIT #{limit}")
    List<ChatSubgroupMsg> getSubgroupMessages(@Param("subgroupId") Integer subgroupId, @Param("limit") Integer limit);

    /**
     * 获取小组消息历史（分页）
     */
    @Select("SELECT * FROM chat_subgroup_msg WHERE subgroup_id = #{subgroupId} AND send_time < #{beforeTime} ORDER BY send_time DESC LIMIT #{limit}")
     ...
     * 搜索小组消息内容
     */
    @Select("SELECT * FROM chat_subgroup_msg WHERE subgroup_id = #{subgroupId} AND msg LIKE CONCAT('%', #{keyword}, '%') ORDER BY send_time DESC LIMIT #{limit}")
