...
public class TokenManager {
    ...
    public TokenModel createToken(String account, String value) {
        ...
        // 删除该用户的其他token
        deleteUserTokens(account, TokenConstant.TOKEN_USER_REDIS);
     ...
     */
    private void deleteUserTokens(String account, String modelName) {
        // 获取所有的key
        String pattern = modelName + "*";
        redisUtil.keys(pattern).forEach(key -> {
            String value = redisUtil.get(key);
            if (value != null) {
                LoginUsername loginUsername = LoginUsername.splitUsername(value);
                // 如果是同一个账号的token，删除它
                if (account.equals(loginUsername.getAccount())) {
